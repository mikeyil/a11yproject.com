...
2. [Visually hide](/posts/how-to-hide-content/) `label` elements (in an accessible manner)
...
In the above CSS, each selector for placeholder text needs to be separate in their own declaration blocks. The reason for this is due to the prefixes needed for placeholder styles to apply. The browser will read the first selector, and if it's not the prefix for that browser, instead of reading through the rest until one applies, it will ignore the rest. To ensure each browser can find it's own prefixed placeholder text styles, it's necessary to write them each separate as we did above.
